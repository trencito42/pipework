...
public struct BoardGeometry: Sendable {
    ...
    public init(gridSize: GridSize, containerSize: CGSize, margin: CGFloat = 16) {
    ...
    /// Converts a screen touch point to the corresponding GridCoord, if inside board bounds.
    public func coord(for point: CGPoint) -> GridCoord? {
        ...
        let col = Int(relX / cellSize)
        let row = Int(relY / cellSize)
